...
jslinkTransform.RenderCollapsibleIndentedTree = function () {
	...
	var color = function (d) {
		return d._children ? "#3182bd" : d.children ? "#775cad" : "#afafaf";
		//console.log(d.children ? d.children.length : 'null');
		// var colorVal = null;
		// if(d._children) {
			// colorVal = '#3182bd';
		// } else if(d.children) {
    ...
    var _convert = function (jsonData) {
		...
		jQuery.each(jsonData["Row"], function(i, val) {
			...
			var item = {
				...
				revId: val.Revision_x003a_RevID,
				acId: val.Revision_x003a_AcId,
				docID: val.Revision_x003a_DocId
			};
			//console.log(item);
			data.push(item);
		});
